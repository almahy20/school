...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!callerIsSuperAdmin && !callerSchoolId) {
      return jsonResponse(req, { error: "Forbidden: caller has no school scope" }, 403);
    }
    ...
    const ensureSameTenant = async (targetUserId: string) => {
      ...
      if (!profileData) {
        return { ok: false, response: jsonResponse(req, { error: "User not found" }, 404), profileData: null };
      }

      if (!callerIsSuperAdmin && profileData.school_id !== callerSchoolId) {
        return {
          ok: false,
          response: jsonResponse(req, { error: "Forbidden: cross-tenant access denied" }, 403),
          profileData,
        };
      }
    ...
    switch (action) {
      ...
      case "delete": {
        if (userId === caller.id) {
          return jsonResponse(req, { error: "Cannot delete yourself" }, 400);
        }
      ...
      case "update_role": {
        if (userId === caller.id) {
          return jsonResponse(req, { error: "Cannot change own role" }, 400);
        }
      ...
      case "update_status": {
        if (userId === caller.id) {
          return jsonResponse(req, { error: "Cannot change own status" }, 400);
        }
      ...
      case "update_status_by_role_id": {
        ...
        if (targetRoleError || !targetRole) {
          return jsonResponse(req, { error: "User role not found" }, 404);
        }
        ...
        if (!callerIsSuperAdmin && targetRole.school_id !== callerSchoolId) {
          return jsonResponse(req, { error: "Forbidden: cross-tenant access denied" }, 403);
        }
      ...
      case "create_user": {
        ...
        if (!phone || !data.password) {
          return jsonResponse(req, { error: "Phone and password are required" }, 400);
        }
        ...
        return jsonResponse(req, { success: true, userId: newUserId });
      }

      case "ban": {
        if (userId === caller.id) {
          return jsonResponse(req, { error: "Cannot ban yourself" }, 400);
        }
