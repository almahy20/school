...
export default function ClassMessagesView({ classId, className }: ClassMessagesViewProps) {
  ...
  // Fetch parent profiles using cached hook
  const { data: parentProfiles } = useProfilesByIds(studentParentIds.length > 0 ? studentParentIds : null);
        ...
        if (rawPhone) {
          ...
          const { data: potentialParents, error: phoneError } = await supabase
            ...
            .or(
              `phone.eq.${rawPhone},` +
              `phone.eq.${cleanPhone},` +
              `phone.eq.+2${cleanPhone.startsWith('0') ? cleanPhone.slice(1) : cleanPhone},` +
              `phone.eq.2${cleanPhone.startsWith('0') ? cleanPhone.slice(1) : cleanPhone}`
            )
            .limit(1);
  ...
  if (selectedStudent) {
    return (
      <div className="space-y-8">
        ...
        ) : parentProfiles && parentProfiles.length > 0 ? (
        ...
        <div className="bg-white p-8 rounded-[32px] border border-slate-100 space-y-6">
          ...
          <Button
            ...
            disabled={sendMessageMutation.isPending || studentParentIds.length === 0}
