...
import { Users, LayoutGrid, ChevronLeft, GraduationCap, Award, CalendarCheck, Wallet } from 'lucide-react';
...
export default function ParentDashboard() {
  ...
  const { data: children = [], isLoading: loading } = useParentChildren();

  if (loading) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center gap-4 animate-pulse">
        <div className="w-12 h-12 border-4 border-slate-100 border-t-indigo-600 rounded-full animate-spin" />
        <p className="text-slate-400 font-bold tracking-widest text-xs uppercase">جاري تحميل بيانات الأبناء...</p>
      </div>
    );
  }
  ...
  if (children.length === 0) {
    return (
      <div className="p-12 sm:p-24 text-center flex flex-col items-center gap-8 bg-white/40 backdrop-blur-md rounded-[56px] border border-white/50 shadow-xl shadow-slate-200/10 max-w-2xl mx-auto my-12 animate-in fade-in zoom-in duration-700">
        <div className="w-24 h-24 rounded-[36px] bg-slate-50 flex items-center justify-center text-slate-200 shadow-inner border border-slate-100">
          <Users className="w-12 h-12" />
        </div>
        <div className="space-y-3">
          <h2 className="text-3xl font-black text-slate-900 tracking-tight">لم يتم العثور على أبناء</h2>
          <p className="text-slate-500 font-medium text-lg leading-relaxed max-w-sm mx-auto">يرجى التواصل مع إدارة المدرسة لربط حسابك بأبنائك وتفعيل ميزة المتابعة الذكية.</p>
        </div>
      </div>
    );
  }
          ...
          >
            <div className="p-10 space-y-8 h-full flex flex-col justify-between relative">
              ...
              <div className="grid grid-cols-3 gap-4">
                 ...
                 <div className="p-4 rounded-2xl bg-slate-50 border border-slate-100 group-hover:bg-rose-50/30 group-hover:border-rose-100/50 transition-colors text-center">
                    <Wallet className="w-5 h-5 text-rose-500 mx-auto mb-2" />
                    <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1">الرسوم المتبقية</p>
                    <p className="text-lg font-black text-slate-900">{child.feesRemaining}</p>
                 </div>
              </div>
