...
export default function ParentChildDetailPage() {
  ...
  const { data: child, isLoading, refetch } = useChildFullDetails(id);
    ...
    return (
      <AppLayout>
        <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
          ...
          <p className="text-slate-400 font-bold text-xs uppercase tracking-widest">جاري تحميل بيانات الطفل...</p>
        </div>
      </AppLayout>
    );
  }
    ...
    return (
      <AppLayout>
        <div className="max-w-4xl mx-auto p-20 text-center">
           <h2 className="text-2xl font-black text-slate-900 mb-4">لم يتم العثور على بيانات</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
               <div className={cn("premium-card p-6 flex flex-col justify-between border shadow-sm", gi.bg)}>
                  ...
                  <div>
                     ...
                     <h3 className={cn("text-3xl font-black leading-none", gi.color)}>{child.avgGrade > 0 ? `${child.avgGrade}%` : '—'}</h3>
                  </div>
               </div>
               ...
               <div className={cn("premium-card p-6 flex flex-col justify-between border shadow-sm", ai.bg)}>
                  ...
                  <div>
                     ...
                     <h3 className={cn("text-3xl font-black leading-none", ai.color)}>{child.attendanceRate > 0 ? `${child.attendanceRate}%` : '—'}</h3>
                  </div>
               </div>
               ...
               <div className="premium-card p-6 flex flex-col justify-between border border-slate-100 bg-slate-50 shadow-sm">
                  ...
                  <div>
                     ...
                     <h3 className="text-3xl font-black leading-none text-slate-900">{child.feesRemaining.toLocaleString()} <span className="text-sm">ر.س</span></h3>
                  </div>
               </div>
            </div>
            ...
            <section className="bg-white border border-slate-100 rounded-[40px] p-2 space-y-6 shadow-sm overflow-hidden">
              ...
              <div className="p-6 pt-0 animate-in fade-in duration-500">
                {tab === 'grades' ? (
                  ...
                  <FinancialView fees={child.fees} payments={child.payments} totalDue={child.feesRemaining} />
                )}
              </div>
            </section>
          </div>
  ...
  return (
    <div className="space-y-8">
      {Object.entries(bySubject).map(([subject, subGrades]) => {
        // Calculate average only for numeric scores
        ...
        const gi = gradeInfo(avg || 100); // Default to Excellent style if only text grades
        return (
      ...
      {totalDue > 0 && (
         <div className="p-8 rounded-[32px] bg-slate-900 text-white flex flex-col md:flex-row items-center justify-between gap-6 shadow-2xl shadow-slate-200">
            ...
            <div className="text-center md:text-left">
               <p className="text-3xl font-black text-white">{totalDue.toLocaleString()} <span className="text-sm opacity-40">ر.س</span></p>
            </div>
         </div>
      )}
            ...
            {payments.map((p: any) => (
              <div key={p.id} className="p-6 rounded-3xl border border-slate-100 bg-white flex items-center justify-between hover:shadow-xl hover:translate-x-[-4px] transition-all group">
                <div className="flex items-center gap-5">
                   ...
                   <div>
                      <p className="text-lg font-black text-slate-900">{p.amount.toLocaleString()} ر.س</p>
                      <div className="flex items-center gap-2 text-[10px] font-bold text-slate-400">
