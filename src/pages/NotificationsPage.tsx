...
export default function NotificationsPage() {
  ...
  useEffect(() => {
    if (autoMarkedRef.current || isLoading || notifications.length === 0) return;
    const hasUnread = notifications.some(n => !n.is_read);
    if (hasUnread) {
      autoMarkedRef.current = true;
      markAllAsReadMutation.mutateAsync().catch(err => {
        logger.error('Error auto-marking as read:', err);
      });
    } else {
      autoMarkedRef.current = true;
    }
